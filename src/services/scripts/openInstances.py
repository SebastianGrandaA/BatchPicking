    ...
    try:
        with open(solutionFile, 'r') as f:
            ...
            for b in range(nbBatches):
                ...
                if len(infos) != 3:
                    handleReadError(fail_fast, solutionFile, line, f"syntax does not fit '<id_{infos[0]}> <nS_{infos[0]}> <nP_{infos[0]}>\\n' paradigm")
                    flag = False

                line = 1+b*3+2
                supportIds = [int(i) for i in f.readline().strip().split(' ')]
                if len(supportIds) != infos[1]:
                    handleReadError(fail_fast, solutionFile, line, f"nb supports={len(supportIds)} is different from nS_{infos[0]}={infos[1]} announced at line {1+b*3+1}")
                    flag = False

                line = 1+b*3+3
                positions = [int(i) for i in f.readline().strip().split(' ')]
                if len(positions) != infos[2]:
                    handleReadError(fail_fast, solutionFile, line, f"nb positions={len(positions)} is different from nP_{infos[0]}={infos[2]} announced at line {1+b*3+1}")
                    flag = False

                batches.append({'id': infos[0], 'supportIds':supportIds, 'positions': positions})
    ...
    try:
        with open(supportListFile, 'r') as f:
            ...
            for s in range(nbSupport):
                ...
                infos = [int(i) for i in f.readline().strip().split(' ')]
                if len(infos) != 3:
                    handleReadError(fail_fast, supportListFile, line, f"syntax does not fit '<id_{infos[0]}> <vol_{infos[0]}> <nP_{infos[0]}>\\n' paradigm")
                    flag = False

                line = 1+s*2+2
                positions = [int(i) for i in f.readline().strip().split(' ')]
                if len(positions) != infos[2]:
                    handleReadError(fail_fast, supportListFile, line, f"nb positions={len(positions)} is different from nP_{infos[0]}={infos[2]} announced at line {1+s*2+1}")
                    flag = False

                supportList.append({'id':infos[0], 'volume':infos[1], 'positions':positions})
    except Exception as e:
        ...
        with open(adjacencyMatrixFile, 'r') as f:
            ...
            for i in range(nbPositions):
                ...
                if len(distances) != nbPositions:
                    handleReadError(fail_fast, adjacencyMatrixFile, line, f"{len(distances)} values given when {nbPositions} were expected")
                    flag = False
    ...
    try:
        with open(constraintsFile, 'r') as f:
            ...
            if maxBatchNbSupports <= 0:
                handleReadError(fail_fast, constraintsFile, line, f"maximum nb supports in a batch is {maxBatchNbSupports}, which is infeasible")
                flag = False

            if maxBatchVolume <= 0:
                handleReadError(fail_fast, constraintsFile, line, f"maximum volume of a batch is {maxBatchVolume}, which is infeasible")
                flag = False
