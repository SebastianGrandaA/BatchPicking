...
from .openInstances import openSolution, openSupportList, openConstraints, openAdjacencyMatrix
...
def evaluate(problemFolder: str, solutionFile: str, stopReadAtFirstError: bool = True, stopCheckAtFirstError: bool = False) -> dict:
    ...
    if not (batches):
        ...
        sys.exit(1)
    elif not checkSolutionCoherence(batches, supports, adjMatrix, maxBatchNbSupports, maxBatchVolume, stopCheckAtFirstError):
    ...
    info(f"Instance {instance_name} | Solution is feasible | Base cost {baseCost} | Objective cost {solutionCost} | Improvement {gain}%")
...
if __name__ == '__main__':
    ...
    """
    parser = argparse.ArgumentParser(description='Solution Checker and evaluator for POIP 2024. Problem folder and solution file are mandatory.')

    parser.add_argument('-p', '--problem_folder', type=str, help='Path to the problem folder', required=True)
    parser.add_argument('-s', '--solution_file', type=str, help='Path to the solution file', required=True)
    ...
    check_options.add_argument('-ffc', '--fail_fast_check', action='store_true', help='Stop program at first solution-checking error found')
    check_options.add_argument('-flc', '--fail_last_check', action='store_true', help='Stop program after all soluction-checking errors are found -> DEFAULT')
